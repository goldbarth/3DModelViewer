...
void Mesh::InitializeBuffers() const
{
    ...
    glGenVertexArrays(BUFFER_SIZE, pVertexBufferObject.get());
    glBindVertexArray(*pVertexBufferObject);
    glGenBuffers(BUFFER_SIZE, pVertexBufferObject.get());
    glBindBuffer(GL_ARRAY_BUFFER, *pVertexBufferObject);
    
    // Generate and bind the Element Buffer Object (EBO)
    glGenBuffers(BUFFER_SIZE, pElementBufferObject.get());
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, *pElementBufferObject);
}
...
std::vector<Vertex> Mesh::TranslateVertices(std::vector<Vertex> vertices, const std::unique_ptr<Transform>& transform)
{
    if (!transform) return vertices;
    for (auto&& vertex : vertices)
    {
        vertex.position.x += transform->position.x;
        vertex.position.y += transform->position.y;
        vertex.position.z += transform->position.z;
